...
{
    /// <summary>
    /// Augments the existing test data with file index which can be appended to the file name to create the unique file
    /// name.
    /// </summary>
    ...
    /// <param name="augmentTestData">
    /// The function which returns test data augmented with the file name index. The same object
    /// or copy can be returned.
    /// </param>
    /// <returns>The test data augmented with the file name index.</returns>
    public static IEnumerable<T> AugmentWithFileIndex<T>(this IEnumerable<T> testData, Func<T, string> getFileName,
        Func<T, int, T> augmentTestData)
    {
