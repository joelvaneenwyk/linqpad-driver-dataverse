...
public static class Context
{
    private static readonly string BaseDir =
        GetDirectoryName(new Uri(Assembly.GetExecutingAssembly().Location).LocalPath)!;
    ...
    private static string? ThrowNotSupportedFrameworkException(bool isNotSupported, string platform)
    {
        return isNotSupported
            ? throw new NotSupportedException(
                $".NET {platform} is not supported. .NET {platform} version is {FrameworkInfo.Version}")
            : null;
    }
    ...
    {
        return WrapCombine(ExeDir, path);
    }
    ...
    {
        return WrapCombine(DataDir, path);
    }
    ...
    {
        return WrapCombine(TemplatesDir, path);
    }
    ...
    {
        return WrapCombine(FilesDir, path);
    }
    ...
    {
        return WrapCombine(BaseDir, path);
    }
